			...
			<div style="height: 300px;; width: 500px; background-color: #A9A9A9; margin: 20px;">
					...
					<%
						for (int i = 1; i <= hmpro.size(); i++) {
								ProfileBean pb= (ProfileBean) hmpro.get(i);
					%>
					<tr>
						<td>
							<%=pb.getLoginid()%>
						...
						<td>
							 <a
								href="AcceptAccount.jsp?id=<%=pb.getLoginid()%>">accept account</a> 
						</td>
						
						<td>
							 <a
								href="RejectAccount.jsp?id=<%=pb.getLoginid()%>">Reject</a> 
						</td>
						<%--<td>
							 <a
								href="DeleteAccount.jsp?id=<%=pb.getLoginid()%>">Delete</a> 
						</td>
					--%></tr>
					<%
						}
